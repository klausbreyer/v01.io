...
- **categories**: always a YAML list with **exactly one entry**. Pick the single most fitting category. Values in use (by frequency): `Bookshelf`, `Engineering`, `Product`, `Personal`, `Process`, `Travel`, `Open Source`, `Speaking`, `Leadership`, `Productivity`, `Remote`, `AI`, `Shipped`. Use `Shipped` for launch posts about something Klaus built and released (an app, a tool, a service). Only use multiple categories if Klaus explicitly asks for it.
...
- **linkedin** (optional): planned LinkedIn post for the article as a `linkedin: |` multiline block (see `2026/08/breadboarding` and `2026/08/franken-domains`). For bilingual posts, put the English text in `index.md` and the German text in `index.de.md`. For launch posts, `2026/08/texttile` is the reference LinkedIn text: a one-line hook, the personal problem, the one-sentence solution, the tech stack in one line, then a call to action and the links to the long version (English and German).
...
- **Launch posts ("Shipped")**: `2026/08/texttile` (`index.md` and `index.de.md`) is the reference for wording and structure when Klaus ships something. Follow it closely: open with the personal scene and the concrete pain, name what existing tools got wrong in one paragraph, then "So I built X" as the turn. Sections: *What it does* (or *What it does differently*) with a short bullet list and screenshots, *What it is not* (deliberately missing features, the "nothing left to take away" philosophy), *Why <tech>* in a few sentences, *Try it* with the install command or store link. Close with a question to the reader. Plain, concrete, no marketing adjectives.
